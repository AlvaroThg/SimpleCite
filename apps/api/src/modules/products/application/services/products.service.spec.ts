...
function makePrisma(opts: {
  ...
  return { prisma: { client } as never, create, update };
}
...
describe('ProductsService', () => {
  ...
  it('archive marca isActive=false (soft delete)', async () => {
    const { prisma, update } = makePrisma({ found: { id: 'p1', stock: 0, tenantId: 't1' } });
    const svc = new ProductsService(prisma);
    const res = await svc.archive('t1', 'p1');
